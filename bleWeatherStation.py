...
sleep = 60
key = 'MO357EK0NHO2W9E0'  # Thingspeak channel to update
...
class WeatherStation:
	...
	def monitorWeatherStation(self):
		...
		if regs is not None:
			# expand Room_AND_CH1_TO_3_TH_DATA_TYPE0
			...
			# expand Room_AND_CH1_TO_3_TH_DATA_TYPE1
	...
	def getRoomTemp(self):
		if 'index0_temperature' in self._data:
			...
			logging.debug('Room temp : %.1f°C, max : %.1f°C, min : %.1f°C', temp, max, min)
			return temp
		else:
			return None
	...
	def getFridgeTemp(self):
		if 'index1_temperature' in self._data:
			...
			logging.debug('Fridge temp : %.1f°C, max : %.1f°C, min : %.1f°C', temp, max, min)
			return temp
		else:
			return None
			
	#Report temperatures to Thingspeak Channel
	def report(self):
    		while True:
        		piTemp = int(open('/sys/class/thermal/thermal_zone0/temp').read()) / 1e3 # Get Raspberry Pi CPU temp
			roomTemp = self.getRoomTemp()
			fridgeTemp = self.getFridgeTemp()
        		params = urllib.urlencode({'field1': fridgeTemp, 'field2': roomTemp, 'field3': piTemp,'key':key }) 
        		headers = {"Content-typZZe": "application/x-www-form-urlencoded","Accept": "text/plain"}
        		conn = httplib.HTTPConnection("api.thingspeak.com:80")
        		try:
            			conn.request("POST", "/update", params, headers)
            			response = conn.getresponse()
            			print roomTemp, fridgeTemp, piTemp
...
class NotificationDelegate(DefaultDelegate):
	def __init__(self):
		...
		self._RoomAndFridgeTemp_type0 = None
		self._RoomAndFridgeTemp_type1 = None
		
	def handleNotification(self, cHandle, data):
		formatedData = binascii.b2a_hex(data)
		if cHandle == 0x0017:
			# RoomAndFridgeTemp indication received
			if formatedData[0] == '8':
				# Type1 data packet received
				self._RoomAndFridgeTemp_type1 = formatedData
				logging.debug('RoomAndFridgeTemp_type1 = %s', formatedData)
			else:
				# Type0 data packet received
				self._RoomAndFridgeTemp_type0 = formatedData
				logging.debug('RoomAndFridgeTemp_type0 = %s', formatedData)
		else:
			# skip other indications/notifications
			logging.debug('handle %x = %s', cHandle, formatedData)
	
	def getData(self):
			if self._RoomAndFridgeTemp_type0 is not None:
				# return sensors data
				return {'data_type0':self._RoomAndFridgeTemp_type0, 'data_type1':self._RoomAndFridgeTemp_type1}
			else:
				return None
	...
	if weatherStationMacAddr is None:
		...
		try:
			...
			if weatherStation.monitorWeatherStation() is not None:
				...
				Room = weatherStation.getRoomTemp()
				Fridge = weatherStation.getFridgeTemp()
